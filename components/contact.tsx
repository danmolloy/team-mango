...
export default function Contact() {
  ...
  return (
    <section id="contact" className=" pt-12 flex flex-col items-center justify-between p-2 py-8 text-sm text-dark w-screen  mx-8 min-h-[80vh] bg-light">
      ...
      <h1 className='self-center font-medium text-6xl text-dark border-t leading-11 border-b border-neutral-300 w-screen font-sans'>CONTACT</h1>
      <p className='px-16 my-4 text-sm font-base '>{`We're always interested to hear about new projects, please feel free to get in touch.`}</p>
      </div>
    ...
    onSubmit={async (values, actions ) => {
      ...
      <Form data-testid="contact-form" className="  flex flex-col  self-center w-full  ">
          ...
          <Field 
            id="name-input" 
            className=" text-dark border rounded-sm  bg-white  w-full p-1 dark:bg-zinc-300"
            type="text"
            name="name"/>
            <div className="h-6">
          ...
          <div className="flex flex-col m-2">
          <label htmlFor='email' className="form-label   font-medium text-sm">Email</label>
          <Field
            id="email" 
            className="text-dark border border-dark bg-white rounded-sm w-full p-1 dark:bg-zinc-300" 
            type="email"
            name="email"/>
            <div className="h-6">
        ...
        <button disabled={props.isSubmitting || (sendStatus === "sending")} id="submit-button" type='submit' className=" disabled:opacity-30 hover:underline    bg-primary text-light m-2 p-1 w-20 rounded-sm self-end cursor-pointer text-sm">SUBMIT</button>
        <div>
        {sendStatus === "sending" 
        ? sendingMsg
        : sendStatus === "err" 
